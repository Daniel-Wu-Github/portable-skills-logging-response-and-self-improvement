...
- Auto-detects the TypeScript package containing the edited file
- Runs `tsc --noEmit --skipLibCheck` on that package (rate-limited to once per 30s per package)
...
- Checks TypeScript errors in any modified package (`tsconfig.json` root detection)
...
When you make a mistake (wrong logic, missed edge case, TypeScript error), append an entry using the template in that file.
